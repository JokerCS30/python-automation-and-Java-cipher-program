...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if receive_first:
        ...
        # if we have data to send to the local client, send it
        if len(remote_buffer):
            print "[<==] sending %d bytes to localhost." % len(remote_buffer)
            client_socket.send(remote_buffer)

    # now loop and read from local, send to remote, send to local, rinse wash and repeat
    while True:
...
# hex dumping function taken from comments here: http://code.activestate.com/recipes/142812-hex-dumper/
...
def receive_from(connection):
    ...
    try:
        # keep reading into the buffer until there's no more data or it times out
